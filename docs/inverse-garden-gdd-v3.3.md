# Inverse Garden: Game Design Document v3.3
...
**Changelog v3.3:**
- Added Phase 1.5: Scale Calibration
- Updated Scale Calculation to use percentile-based mapping (fixes clustering issue)
- Adjusted sprout scale range for visibility (was 0.4-0.8, now 0.8-1.0)
- Added Percentile Calculation Reference in Technical Reference section
...
**Future enhancement (Phase 4):** Generate these using deterministic pseudo-randomization seeded by the entry timestamp, with the defaults as midpoints and ±20-30% variation. This would add visual variety between entries while ensuring the same entry always produces an identical plant on revisit/reload.

### Scale Calculation (Percentile-Based)

**Problem:** Raw valence values cluster heavily (many entries near -0.5 to -0.7), causing most plants to look similar in size when using linear scaling.

**Solution:** Use percentile-based mapping to ensure visual distinction regardless of data clustering.

**How it works:**

1. **Pre-calculate percentiles** for each component type separately:
   - Sort all flower entries by |valence|, assign percentile ranks (0-100)
   - Sort all decay entries by |valence|, assign percentile ranks (0-100)
   - Sprouts (neutral) have minimal variation, use fixed scale

2. **Map percentile to scale:**
   ```
   scale = minScale + (percentile / 100) × (maxScale - minScale)
   ```

3. **Result:** Roughly 10% of flowers will be at each size tier, ensuring visual variety even with clustered data.

**Scale Ranges:**

| Component | Min Scale | Max Scale | Notes |
|-----------|-----------|-----------|-------|
| Flower | 0.7 | 1.5 | Full range of bloom sizes |
| Sprout | 0.8 | 1.0 | Narrow range, always visible |
| Decay | 0.8 | 1.8 | Size = diameter of ground scar |

**Implementation note:** Percentiles should be calculated once at data load time and stored with each entry, not recalculated on every render. See Technical Reference for code example.
...
- Adjust sprout scale range for visibility (0.8 - 1.0)
- Visual validation with full dataset
- Tune min/max scale values until size distribution looks good

**Success criteria:** Plants show clear size variation; sprouts are visible; no two adjacent entries look identical in size.
...
- Plant parameter variance (seeded randomness for visual variety)
...
|-----------|---------------|-------|
| Flower scale range | 0.7 - 1.5 | Percentile-mapped |
| Sprout scale range | 0.8 - 1.0 | Narrow range for visibility |
| Decay scale range | 0.8 - 1.8 | Percentile-mapped |
...
```
/Mood Garden Build/
├── inverse-garden-gdd-v3.3.md        # This document (the spec)
...
export interface FlowerDNA {
  ...
  scale: number;               // 0.7-1.5 (calculated from percentile)
...
export interface SproutDNA {
  ...
  scale: number;               // 0.8-1.0 (narrow range for visibility)
}
...
export interface DecayDNA {
  ...
  // Size & Shape
  size: number;                // 0.8-1.8 (calculated from percentile)
...
### Percentile Calculation Reference
...
interface EntryWithPercentile extends MoodEntry {
  scalePercentile: number;  // 0-100, calculated at load time
}

// Helper functions
function isFlower(classification: string): boolean {
  return ['Very Unpleasant', 'Unpleasant', 'Slightly Unpleasant'].includes(classification);
}

function isDecay(classification: string): boolean {
  return ['Very Pleasant', 'Pleasant', 'Slightly Pleasant'].includes(classification);
}
...
// Main percentile calculation
function calculatePercentiles(entries: MoodEntry[]): EntryWithPercentile[] {
  ...
  // Sort each group by |valence| and assign percentiles
  const assignPercentiles = (group: MoodEntry[]): EntryWithPercentile[] => {
    if (group.length === 0) return [];
    if (group.length === 1) return [{ ...group[0], scalePercentile: 50 }];
  ...
  return [
    ...assignPercentiles(flowers),
    ...assignPercentiles(decays),
    ...sprouts.map(e => ({ ...e, scalePercentile: 50 }))  // Fixed middle value for sprouts
  ];
}

// Convert percentile to actual scale value
function percentileToScale(percentile: number, min: number, max: number): number {
  return min + (percentile / 100) * (max - min);
}

// Scale ranges
const SCALE_RANGES = {
  flower: { min: 0.7, max: 1.5 },
  sprout: { min: 0.8, max: 1.0 },
  decay:  { min: 0.8, max: 1.8 }
};

// Usage example
function getScale(entry: EntryWithPercentile): number {
  if (isFlower(entry.valenceClassification)) {
    return percentileToScale(entry.scalePercentile, SCALE_RANGES.flower.min, SCALE_RANGES.flower.max);
  } else if (isDecay(entry.valenceClassification)) {
    return percentileToScale(entry.scalePercentile, SCALE_RANGES.decay.min, SCALE_RANGES.decay.max);
  } else {
    return percentileToScale(entry.scalePercentile, SCALE_RANGES.sprout.min, SCALE_RANGES.sprout.max);
  }
}
```
...
2026-01-07 10:44:27 -0500,2026-01-07 10:44:27 -0500,Momentary Emotion,Indifferent,,0.09349590394349590394555074,Neutral,
...
- `inverse-garden-gdd-v3.2.md` — Pre-percentile scaling version
